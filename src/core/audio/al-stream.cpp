...
float ALStream::queryOffset()
{
    ...
    float procOffset = static_cast<float>(procFrames) / source->sampleFreq();
...
struct ALStreamOpenHandler : FS::OpenHandler
{
    ...
    ALStreamOpenHandler(SDL_IOStream &src_io, bool looped)
        : src_io(&src_io), looped(looped), source(0), fallbackMode(0)
    {}
    ...
    bool tryRead(SDL_IOStream &io, const char *ext)
    {
        ...
        char sig[5] = { 0 };
        SDL_ReadIO(src_io, sig, 4);
        SDL_SeekIO(src_io, 0, SDL_IO_SEEK_SET);

        try
        {
            if (!strcmp(sig, "OggS"))
            {
                source = createVorbisSource(*src_io, looped);
                return true;
            }

            source = createSDLSource(*src_io, ext, STREAM_BUF_SIZE, looped, fallbackMode);
        }
...
{
    ALStreamOpenHandler handler(*src_io, looped);
...
void ALStream::startStream(float offset)
{
    ...
    startOffset = offset;
    procFrames = offset * source->sampleFreq();
